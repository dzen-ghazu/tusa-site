...
After a partial refund, the order status stays **Completed** (it's only marked Refunded when the _entire_ order is refunded). The order details show what was refunded and the original total alongside.
...
**Important:** because Stripe pays out to you within the day, refunds come from your _current_ Stripe balance — not from a holding account. If you've already withdrawn the money or spent it on club costs, refunding may push your balance negative. See [Where the refund money comes from](#where-the-refund-money-comes-from) below.
...
- **Larger refunds need bigger thinking.** Refunding 30 attendees from a \$20-each event = \$600 going back. Talk to TUSA _before_ you process — they'll know if your Stripe balance can cover it and what your options are if not.
